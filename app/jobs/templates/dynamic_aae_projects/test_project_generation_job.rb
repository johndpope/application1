...
module Templates
	module DynamicAaeProjects
		TestProjectGenerationJob = Struct.new(:dynamic_aae_project_id) do
			def perform
				...
				ActiveRecord::Base.transaction do
					unless dynamic_aae_project.tar_project.present?
						begin
							...
							raise "Test Dynamic AAE Project must have target=test, current target=#{dynamic_aae_project.target}" unless dynamic_aae_project.target.test?
