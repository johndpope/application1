$(function () {
    ...
    if (!$(this).hasClass("open")) {
      ...
      $(this).removeClass("open");
		...
		if (typeof(Storage) != 'undefined') {
			if (open) {
				localStorage.setItem('text-chunks-filter-settings-open', 'true');
			} else {
				localStorage.setItem('text-chunks-filter-settings-open', 'false');
			}
		} else {
	...
	if (typeof(Storage) != 'undefined' && localStorage.getItem('text-chunks-filter-settings-open') == 'true') {
    filter.animate({ 'right': '250px' });
    filter_settings.animate({ 'right': '0' });
    filter.addClass("open");
	}

	$('#text_chunks_table th').on('click', function () {
		if ($(this).hasClass('sort')) {
			var data_field = $(this).attr('data-field');
			if ($(this).hasClass('sort_asc')) {
				$('#text_chunks_table th').removeClass('sort_asc').removeClass('sort_desc');
