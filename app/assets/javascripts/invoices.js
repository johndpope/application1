$(function () {
  ...
  filter.click(function() {
    ...
    if (typeof(Storage) != "undefined") {
      if (open) {
        localStorage.setItem("invoices-filter-settings-open", "true");
      } else {
        localStorage.setItem("invoices-filter-settings-open", "false");
      }
    } else {
  ...
  if (typeof(Storage) != "undefined" && localStorage.getItem("invoices-filter-settings-open") == "true") {
    filter.animate({ 'right': '250px' });
    filter_settings.animate({ 'right': '0' });
    filter.addClass("open");
  }

  $("#invoices_table th").on("click", function() {
    if($(this).hasClass("sort")) {
      var data_field = $(this).attr("data-field");
      if ($(this).hasClass("sort_asc")) {
        $("#invoices_table th").removeClass("sort_asc").removeClass("sort_desc");
  ...
  $('body').on('click', '*[data-legend-url]', function (event) {
		...
		element = $(this);
		$('#invoice_legend').empty();
		$.ajax({ url: element.data('legend-url') }).done(function (response) {
			$('#invoice_legend').append(response).modal();
		});
	});
});
