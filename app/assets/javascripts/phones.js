$(function () {
  ...
  $('.select2-container').addClass('form-control');
    ...
    if (!$(this).hasClass("open")) {
      ...
      $(this).removeClass("open");
		...
		if (typeof(Storage) != 'undefined') {
			if (open) {
				localStorage.setItem('phones-filter-settings-open', 'true');
			} else {
				localStorage.setItem('phones-filter-settings-open', 'false');
			}
		} else {
	...
	if (typeof(Storage) != 'undefined' && localStorage.getItem('phones-filter-settings-open') == 'true') {
    filter.animate({ 'right': '250px' });
    filter_settings.animate({ 'right': '0' });
    filter.addClass("open");
	}

	$('#phones_table th').on('click', function () {
		if ($(this).hasClass('sort')) {
			var data_field = $(this).attr('data-field');
			if ($(this).hasClass('sort_asc')) {
				$('#phones_table th').removeClass('sort_asc').removeClass('sort_desc');
